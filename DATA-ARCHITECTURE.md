...
5. The JND color clustering is a downstream step that **rewrites** the JSON — `python Data_Pipeline/3_Clustering/recluster_at_jnd.py` clusters the 6,444 configs to 202 anchors (two-axis CIEDE2000: exterior ≤1.5 + transmitted ≤3.0) and **injects a 1-based integer `cid` (anchor id) onto every record**. It is therefore the *final* writer of `App_Data/*.json`: re-running `csv_to_json.py` alone drops `cid`, so always re-run the clustering after it. See `Data_Pipeline/3_Clustering/CLUSTERING_PROCEDURE.md`.
